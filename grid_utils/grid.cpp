...
Grid::Grid(const Grid *g, bool copy_colors) {
  ...
  if (!copy_colors) {
    ...
    this->colors = copy_ttab_char((const char **)g->colors, this->size,
...
/// @param test set the Grid for testing, if true the filename will be
/// {filename}[N|C]{extension} where N reprensents the numbers and C the colors
/// of each files
Grid::Grid(std::string filename, std::string extension, bool test = false) {
  if (test) {
    ...
    if (is_valid_score()) {
      ...
      std::cout << "The score was wrongly calculated, the score calculated is "
                << score << " but the supposed score is " << supposed_score
                << ".\n";
    }
  } else {
...
int Grid::get_cross_score(int line, int col) const {
  return get_number(line, col) + get_number(line, col - 1) +
         get_number(line, col + 1) + get_number(line - 1, col) +
         get_number(line + 1, col);
}

int ***Grid::build_scores_tab(int black, int green, int yellow, int red,
                              bool all) const {
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      ...
      infoTab[i][j][yellow] =
          calcul_score_place(i, j, 'J', true) /* - penality */;
...
int Grid::glouton(int value) {
  ...
  int nb_black = nb_color_in_grid('N'),
      placed_pieces = max_pieces - nb_empty_cells(),
      nb_red = nb_color_in_grid('R');

  do {
    max_cur = value;
    color_cur = -1;
    coordinates_max = couple{0, 0};
    // iterate through all table to find the max value, green should be more
    // considered
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        // cell already taken
        ...
        for (int k = 0; k < 4; k++) {
          ...
          if (scores_tab[i][j][k] < max_cur or (k == red and nb_red >= 1)) {
            // there is this condition to optimize the code (instead of checking
            // two there is only on checked which coule be more true)
            continue;
    ...
    if (max_cur != value) {
      ...
      case yellow:
        ...
        this->update_yellow_scores_tab(scores_tab, coordinates_max.line,
                                       coordinates_max.column, yellow);
      ...
      case green:
        ...
        this->update_green_scores_tab(scores_tab, coordinates_max.line,
                                      coordinates_max.column, green);
    ...
    for (int x = 0; x < size_choice; x++) {
      ...
      for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
          // cell already taken
          if (colors[i][j] != 'X' or
              contains_couple(cl, chosen_pieces_size, i, j)) {
            continue;
          }
          ...
          for (int k = 0; k < 4; k++) {
            // handle priorities between colors
            if (scores_tab[i][j][k] < max_cur or
                (k == black and nb_black >= size) or
                (k == red and nb_red >= 1)) {
              // there is this condition to optimize the code (instead of
              // checking two there is only on checked which coule be more true)
              continue;
            } else if (scores_tab[i][j][k] > max_cur) {
              max_cur = scores_tab[i][j][k];
              color_cur = k;
...
// this function does not generate the best grid, the around of green is not
// really taken in mind
void Grid::build_grid_points(options *opts) {
  ...
  for (int i = 0; i < 1000; i++) {
    boucle_alea(nb_placed_glouton, opts);
  }
  std::cout << "Final version of the grid : \n";
  this->set_score_from_calculation();
  this->print_colors_with_score();
  if (opts->output_file != nullptr) {
    this->save_in_file(opts->output_file);
  }
}

void Grid::boucle_alea(int nb_placed_glouton, options *opts) {
...
void Grid::find_value(int value, options *opts) {
  Grid *bestGrid = this->copy_grid_as_ptr(true);
  bestGrid->build_grid_points(opts);
  if (bestGrid->get_score() < value) {
    std::cout << "The given value is higher than the best value that we can "
                 "compute\n";
    exit(0);
  }
}
    ...
    for (int j = 0; j < size; ++j) {
      if ((i != a) or (j != b)) {
        ...
        if (n >= 0 and n < black) {
          ...
          p[i][j] = 'J';
          yellow--;
        } else if (n >= black + blue + yellow and
...
void Grid::optimize_grid_full() {
  ...
  int score_yellow = INT32_MIN, score_black = INT32_MIN, score_blue = INT32_MIN,
      score_green = INT32_MIN, score_orange = INT32_MIN;
  ...
  for (int i = 0; i < size; ++i) {
    for (int j = 0; j < size; ++j) {
      ...
      if (col == 'B' or col == 'O' or scores_tab[i][j][co] < 0 or
          (c[i][j] > 0 and scores_tab[i][j][co] < penality)) {
        if (p[i][j] != 'J') {
          p[i][j] = 'J';
...
/// @brief optimize recursively the grid, to be known the grid that calls this
/// function will be added to glg
/// @param glg the given GridLinkGuard that stores the Grids generated by the
/// optimization
/// @param cl a list structure to store the coordinates that should be tested,
/// avoiding repeated branches
/// @param limit_recur 2 => couple modified, 3 => triple, 4 => ...
void Grid::optimize_grid_recur(GridLinkGuard *glg, coupleLink *cl,
                               int limit_recur) {
    ...
    for (int j = 0; j < size; ++j) {
      if (p[i][j] == 'B' and numbers[i][j] < 0) {
        ...
        if (b >= 1) {
              ...
              for (int s = 0; s < size; s++) {
                ...
                if (p[r][s] == 'B' and numbers[r][s] > 0) {
                  ...
                  if (d >= 1) {
                    ...
                    if (get_color(r + 1, s - 1) == 'B' and
                        numbers[r + 1][s - 1] > 0 and d >= 1 and not changed) {
                      if ((c[r][s] + c[r + 1][s - 1] + c[i][j] + c[i][j + 1]) >
                          0) {
                        p[r][s] = 'J';
                        p[r + 1][s - 1] = 'J';
                        p[i][j] = 'J';
                    ...
                    if (get_color(r + 1, s) == 'B' and numbers[r + 1][s] > 0 and
                        d >= 1 and not changed) {
          ...
          if (get_color(i + 1, j - 1) == 'B' and numbers[i + 1][j - 1] < 0 and
              b >= 1) {
            changed = false;
            for (int r = 0; r < size; r++) {
              for (int s = 0; s < size; s++) {
                ...
                if (p[r][s] == 'B' and numbers[r][s] > 0) {
                  ...
                  if (d >= 1) {
                    if (get_color(r, s + 1) == 'B' and numbers[r][s + 1] > 0) {
                      if ((c[r][s] + c[r][s + 1] + c[i][j] + c[i + 1][j - 1]) >
                          0) {
                        p[r][s] = 'J';
                        p[r][s + 1] = 'J';
                        p[i][j] = 'J';
                    ...
                    if (get_color(r + 1, s) == 'B' and numbers[r + 1][s] > 0 and
                        d >= 1 and not changed) {
                      if ((c[r][s] + c[r + 1][s] + c[i][j] + c[i + 1][j - 1]) >
                          0) {
                        p[r][s] = 'J';
                        p[r + 1][s] = 'J';
                        p[i][j] = 'J';
              ...
              for (int s = 0; s < size; s++) {
                ...
                if (p[r][s] == 'B' and numbers[r][s] > 0) {
                  ...
                  if (d >= 1) {
                    ...
                    if (get_color(r + 1, s - 1) == 'B' and
                        numbers[r + 1][s - 1] > 0 and d >= 1 and not changed) {
                      if ((c[r][s] + c[r + 1][s - 1] + c[i][j] + c[i + 1][j]) >
                          0) {
                        p[r][s] = 'J';
                        p[r + 1][s - 1] = 'J';
                        p[i][j] = 'J';
