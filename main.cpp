...
{
    std::cout << "Welcome in the home page : \n\nThis command has to way to be used :\n\n";
    ...
    std::cout << "=> -w / --write-all : Write all best solutions found into several files\n";
    std::cout << "\n2) GRID GENERATOR\n\n";
    std::cout << "./exec -g / --generate-numbers [-h | p | s | m ] OUTPUT_FILE , the following options are mandatory\n\n";
    std::cout << "=> -g / --generate-numbers : mandatory for the generation of the random numbers of a grid\n";
    std::cout << "=> -s / --size : specify the size of the grid to be generated\n";
    std::cout << "=> -p / --penality : specify the penality of the grid to be generated\n";
    ...
    *solve_grid = true;
    ...
    else
    {
        ...
        else
        {
            ...
            {
                std::cerr << "For the generation of a random grid filled with numbers, please provide a size and a penality with the options -s and -p, you also have to set the options -m for the minimum and the maximum values\n";
                exit(0);
            }
